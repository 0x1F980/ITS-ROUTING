...
use its_transport::masking::{encapsulate, decapsulate};
use its_transport::otm::{
    generate_tag, verify_tag, derive_forward_secret,
    verify_forward_share, verify_backward_share,
};
use its_transport::poly::Polynomial;
...
fn test_pure_geometric_tunnel_1000_steps() {
    ...
    let mut ciphertexts = [0 as its_transport::field_arith::FieldStorage; 1000];
    ...
    for i in 1..=1000 {
        ...
        let bob_nonce = its_transport::trapdoor::lagrange_interpolate(&points_back_reconstructed, FieldElement::new(2147483646));
        ...
        let bob_k_mac = its_transport::trapdoor::lagrange_interpolate(&points_forw_reconstructed, FieldElement::new(2147483645));
    ...
    for &c in ciphertexts.iter() {
        let bin = (((c as u128) * 10) / (its_transport::field_arith::MODULUS as u128)).min(9) as usize;
        counts[bin] += 1;
    }
...
fn test_active_attacker_mitm_and_replay() {
    ...
    // =========================================================================
    {
        ...
        let _bob_k_mac = its_transport::trapdoor::lagrange_interpolate(&points_forw_reconstructed, FieldElement::new(2147483645));
