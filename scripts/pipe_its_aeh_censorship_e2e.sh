#!/usr/bin/env bash
# AEH last-resort E2E gate: same epoch cells as pool, embedded φ ~ D_benign (no daemon/UDP).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ASYM="${ITS_ASYMMETRIC_DIR:-/home/user/ITS-asymmetric}"
TMP="${TMPDIR:-/tmp}/its_aeh_e2e_$$"
POOL="$TMP/pool"
mkdir -p "$TMP" "$POOL"
trap 'rm -rf "$TMP"' EXIT
...
MSG="its-aeh e2e $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
echo "== client-send --aeh (last-resort, manual) =="
"$ROUTING" -c "$TMP/aeh.toml" client-send --aeh --file "$TMP/msg.wire" \
  --ratchet-seed-file "$TMP/ratchet.seed"

echo "== client-receive --aeh =="
"$ROUTING" -c "$TMP/aeh.toml" client-receive --aeh \
  --ratchet-seed-file "$TMP/ratchet.seed" --out "$TMP/recv.wire"

if [[ ! -f "$TMP/recv.wire" ]]; then
  echo "AEH receive FAILED: no output" >&2
  exit 1
fi
...
# Release build must not contain WIKI_STEGO strings
if strings "$ROUTING" | grep -q 'WIKI_STEGO'; then
  echo "FAIL: WIKI_STEGO found in release binary" >&2
  exit 1
fi

echo "pipe_its_aeh_censorship_e2e.sh: OK ($MSG)"
