#!/usr/bin/env bash
# KM pool E2E: its-km send → its-km receive (F4 + vault transport_ratchet).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ASYM="${ITS_ASYMMETRIC_DIR:-/home/user/ITS-asymmetric}"
KM="${ITS_KM_DIR:-/home/user/ITS-KeyManagement}"
TMP="${TMPDIR:-/tmp}/its_km_pool_e2e_$$"
POOL="$TMP/pool"
...
# Alice's vault: contact bob — encrypt with bob's public key
...
MSG="km-pool e2e $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
# Bob's vault: contact alice (alice.pk + bob.sk)
...
[[ "$(cat "$TMP/out.txt")" == "$MSG" ]]
echo "pipe_its_km_pool_e2e.sh: OK"
