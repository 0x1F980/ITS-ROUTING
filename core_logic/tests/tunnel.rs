...
impl SimpleRng {
    ...
    fn next_field_element(&mut self) -> FieldElement {
        // Modulo 17 reduktion for at få et uniformt felt-element
        FieldElement::new((self.next_u32() % 17) as u8)
    }

    fn next_non_zero_field_element(&mut self) -> FieldElement {
        // Genererer et element i Z_17 \ {0}
        FieldElement::new(((self.next_u32() % 16) + 1) as u8)
    }
}
...
fn test_pure_geometric_tunnel_1000_steps() {
    ...
    // =========================================================================
    // Modulus p = 17. Threshold K = 2 (lineære polynomier).
    ...
    // Alice og Bob deler det statiske baglæns-polynomium Q(x) = 5 + 3x (modulo 17)
    ...
    for i in 1..=1000 {
        ...
        // Vi roterer x_i i feltet Z_17 (undgår x=0 og x=1 for at forhindre overlap med Master Root og public point)
        let x_i = FieldElement::new(((i % 15) + 2) as u8);
        let y_back = poly_backward.evaluate(x_i);
        let alice_back_point = (x_i, y_back);

        // Alice udleder det hemmelige nonce N_i ved at evaluere Q(16) (en hemmelig indeks Eve aldrig ser)
        let nonce = poly_backward.evaluate(FieldElement::new(16));

        // B. Forlæns SSS (Integritet)
        // Alice udleder den forlæns hemmelighed fra det foregående baglæns punkt og besked
        ...
        // b_i er en frisk hældning genereret fra hendes TRNG
        let b_i = rng.next_non_zero_field_element();
        let poly_forward = Polynomial::new([s_forw, b_i]);

        // Alice evaluerer P_i(x) ved beskeden M_i for at danne sit forlæns integritetspunkt
        let y_forw = poly_forward.evaluate(secret_msg);
        let alice_forw_point = (secret_msg, y_forw);

        // Alice udleder den hemmelige MAC-nøgle K_MAC ved at evaluere P_i(15) (hemmelig indeks)
        let k_mac = poly_forward.evaluate(FieldElement::new(15));

        // C. OTP Maskering & Tag-generering
        // Alice genererer en frisk OTP-nøgle K_pool fra sin TRNG
        ...
        let bob_nonce = core_logic::trapdoor::lagrange_interpolate(&points_back_reconstructed, FieldElement::new(16));
        ...
        let bob_k_mac = core_logic::trapdoor::lagrange_interpolate(&points_forw_reconstructed, FieldElement::new(15));
    ...
    // =========================================================================
    // Vi tjekker, om de 1.000 ciphertexts er uniformt fordelt over Z_17.
    let mut counts = [0u32; 17];
    for &c in ciphertexts.iter() {
        counts[c as usize] += 1;
    }

    let expected = 1000.0 / 17.0; // ~58.82 pr. bin
    ...
    // For df = 16 og alpha = 0.01 er den kritiske værdi ~32.0.
    // Hvis chi_squared < 32.0, kan vi ikke skelne ciphertexts fra perfekt uniform støj!
...
fn test_active_attacker_mitm_and_replay() {
    ...
    // =========================================================================
    {
        ...
        let _bob_k_mac = core_logic::trapdoor::lagrange_interpolate(&points_forw_reconstructed, FieldElement::new(15));
    ...
    // =========================================================================
    {
        ...
        // Da Bobs s_forw er anderledes, vil den rekonstruerede k_mac være fuldstændig korrupt,
        // og tag-verifikationen vil fejle prompte!
        let points_back_reconstructed = [
            (FieldElement::zero(), master_root),
            alice_back_point,
        ];
