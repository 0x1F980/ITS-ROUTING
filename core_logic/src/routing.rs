...
impl<const K: usize> HydraNode<K> {
    ...
    pub fn process_packet<R: SecureRandom>(
        &self,
        packet: &HydraOnionPacket,
        hop_index: usize, // 0, 1, or 2 representing the current hop layer
        rng: &mut R,
    ) -> Result<(FieldElement, HydraOnionPacket), ()> {
        if hop_index >= 3 {
            return Err(());
        }
    ...
    pub fn queue_packet(&mut self, packet: HydraOnionPacket) {
...
mod tests {
    ...
    fn test_onion_routing_3_hops_roundtrip() {
        ...
        let node1 = HydraNode::new(FieldElement::new(1), trapdoor_1, mac_key_1, nonce_1);
        let node2 = HydraNode::new(FieldElement::new(2), trapdoor_2, mac_key_2, nonce_2);
        let node3 = HydraNode::new(FieldElement::new(3), trapdoor_3, mac_key_3, nonce_3);
