...
pub struct Sel4SharedPage {
    ...
    ///
    /// Layout:
    /// - Bytes 0..2: masked_point (x, y)
    /// - Bytes 2..4: forward_point (x, y)
    /// - Bytes 4..6: backward_point (x, y)
    /// - Byte 6: tag
    /// - Byte 7: ciphertext
    pub data: [u8; 8],
...
impl Sel4SharedPage {
    ...
    pub fn serialize_packet(&mut self, packet: &ScpstPacket) {
        self.data[0] = packet.masked_point.0.value();
        self.data[1] = packet.masked_point.1.value();
        self.data[2] = packet.forward_point.0.value();
        self.data[3] = packet.forward_point.1.value();
        self.data[4] = packet.backward_point.0.value();
        self.data[5] = packet.backward_point.1.value();
        self.data[6] = packet.tag.value();
        self.data[7] = packet.ciphertext.value();
    }
    ...
    pub fn deserialize_packet(&self) -> ScpstPacket {
        ScpstPacket {
            masked_point: (
                FieldElement::new(self.data[0]),
                FieldElement::new(self.data[1]),
            ),
            forward_point: (
                FieldElement::new(self.data[2]),
                FieldElement::new(self.data[3]),
            ),
            backward_point: (
                FieldElement::new(self.data[4]),
                FieldElement::new(self.data[5]),
            ),
            tag: FieldElement::new(self.data[6]),
            ciphertext: FieldElement::new(self.data[7]),
        }
    }
}
...
///
/// This function verifies that the raw pointer is non-null and that the
/// memory range is valid before performing any read or write operations.
#[inline]
...
/// - `poly_coeffs`, `public_point`, and `initial_back` are valid, readable pointers pointing to arrays of at least 2 bytes.
#[no_mangle]
pub unsafe extern "C" fn scpst_alice_init(
    endpoint_buf: *mut AliceEndpoint<2>,
    poly_coeffs: *const u8,
    public_point: *const u8,
    initial_back: *const u8,
    initial_msg: u8,
) -> i32 {
...
/// - `trapdoor_points` points to a valid, readable array of at least 4 bytes (representing 2 points * 2 coordinates).
/// - `initial_back` points to a valid, readable array of at least 2 bytes.
#[no_mangle]
pub unsafe extern "C" fn scpst_bob_init(
    endpoint_buf: *mut BobEndpoint<2>,
    master_root: u8,
    trapdoor_points: *const u8,
    initial_back: *const u8,
    initial_msg: u8,
) -> i32 {
    ...
    let td_pts = slice::from_raw_parts(trapdoor_points, 4); // 2 points * 2 coordinates = 4 bytes
    let init_back = slice::from_raw_parts(initial_back, 2);
...
///
/// In seL4, hardware access is restricted. A component might need to call a
/// microkernel capability or another component to get entropy. This structure
/// wraps a C callback function to implement the `SecureRandom` trait.
pub struct FfiRng {
...
///
/// # Arguments
/// * `endpoint` - Pointer to Alice's initialized endpoint.
/// * `message` - The secret message byte.
/// * `shared_page` - Pointer to the seL4 shared memory page where the packet will be written.
/// * `trng_callback` - C callback function to supply entropy.
///
/// # Safety
/// This function is unsafe because it dereferences raw pointers. The caller must ensure that:
...
///
/// # Arguments
/// * `endpoint` - Pointer to Bob's initialized endpoint.
/// * `shared_page` - Pointer to the seL4 shared memory page containing the incoming packet.
/// * `decrypted_message_out` - Pointer where the decrypted message byte will be written.
///
/// # Safety
/// This function is unsafe because it dereferences raw pointers. The caller must ensure that:
...
/// - `decrypted_message_out` points to a valid, writable memory location of at least 1 byte.
#[no_mangle]
pub unsafe extern "C" fn scpst_bob_receive(
    endpoint: *mut BobEndpoint<2>,
    shared_page: *const Sel4SharedPage,
    decrypted_message_out: *mut u8,
) -> i32 {
